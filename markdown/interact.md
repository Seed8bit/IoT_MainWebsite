...
<p>上图描述了锐客创新物联网平台的典型应用。用户从以太网或SD卡输入指令，开发板根据指令会发送硬件操作命令给执行器，比如开启电机转动。当执行器完成任务后，开发板发送硬件操作命令给传感器，收集传感器的数据。最后返回结果。</p>
...
## 硬件操作
<p>物联网平台支持对GPIO，ADC，SPI，I2C，UART，PWM，时钟模块，文件系统的硬件操作。</p>
